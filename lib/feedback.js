...
(function () {
  ...
  function renderPending() {
    ...
    if (lastSubmittedBatch) {
      ...
      // Per-comment progress: which of this batch's comments are still waiting.
      const outstanding = new Set(batchOutstandingIds());
      const doneCount = n - outstanding.size;
      const partial = doneCount > 0 && outstanding.size > 0;
      const submittedList = lastSubmittedBatch.pending_snapshot.map(c => {
        const waiting = outstanding.has(c.id);
        const tag = waiting
          ? `<span class="cf-comment-pending-tag">awaiting</span>`
          : `<span class="cf-comment-done-tag">✓ addressed</span>`;
        return `<div class="cf-comment-quote${waiting ? "" : " cf-comment-quote-done"}" style="margin-top:4px;">${tag} ${escapeHtml(c.comment)}</div>`;
      }).join("");

      if (isBatchStale) {
        const staleHeadline = partial
          ? `${outstanding.size} of ${n} comment${n === 1 ? "" : "s"} not addressed`
          : "No agent picked this up yet";
        const staleStatus = partial
          ? `The agent answered ${doneCount} of ${n} comment${n === 1 ? "" : "s"} in this batch but left ${outstanding.size} unaddressed (still in <code>feedback/inbox.jsonl</code>). Re-send the awaiting ones, or ask the running session to "address the rest of the pending feedback".`
        ...
        banner.innerHTML = `
          <div class="cf-processing-row">
            ...
            <div class="cf-processing-body">
              <strong>${staleHeadline}</strong>
              <span class="cf-processing-meta">submitted ${submittedAgo}</span>
            </div>
          </div>
        ...
        const headline = partial
          ? `Claude is processing… (${doneCount}/${n} done)`
          : "Claude is processing…";
        const meta = partial
          ? `${outstanding.size} of ${n} comment${n === 1 ? "" : "s"} still awaiting · submitted ${submittedAgo}`
          : `${n} comment${n === 1 ? "" : "s"} · submitted ${submittedAgo}`;
        banner.innerHTML = `
          <div class="cf-processing-row">
            <div class="cf-spinner" aria-hidden="true"></div>
            <div class="cf-processing-body">
              <strong>${headline}</strong>
              <span class="cf-processing-meta">${meta}</span>
            </div>
          </div>
          <details class="cf-processing-details"${partial ? " open" : ""}>
            <summary>show what you submitted</summary>
            ${submittedList}
          </details>
        `;
  ...
  // Set of every comment id that some history change claims to address.
  function answeredCommentIds() {
  ...
  // Comment ids from the submitted batch that NO history change addresses yet.
  function batchOutstandingIds() {
    if (!lastSubmittedBatch) return [];
    const answered = answeredCommentIds();
    return lastSubmittedBatch.comment_ids.filter((id) => !answered.has(id));
  }

  // A batch is processed only when EVERY comment in it has been addressed.
  // Previously this returned true as soon as ANY single comment was answered,
  // which cleared the whole "processing" state and silently dropped the rest
  // of a multi-comment batch from the UI. Per-comment tracking keeps the
  // unaddressed comments visible until they're actually handled.
  function lastBatchProcessed() {
    if (!lastSubmittedBatch) return true;
    return batchOutstandingIds().length === 0;
  }
  ...
  function onHistoryUpdated() {
    ...
    if (lastSubmittedBatch && lastBatchProcessed()) {
      ...
      saveLS();
      renderPending();
    } else if (lastSubmittedBatch) {
      // Still waiting on some comments — re-render so the "X/Y done" progress
      // and the per-comment awaiting/addressed marks update as answers land.
      renderPending();
    }
