...
(function () {
  ...
  // Edit-mode state — when on, text-bearing leaf elements become
  // contenteditable. On blur, the changed text is POSTed to /edit and the
  // server rewrites the HTML file directly (plain text only).
  let editMode = false;
  const editOriginalText = new WeakMap(); // el -> original textContent when focused
  ...
  function buildUI() {
    ...
    root.innerHTML = [
      ...
      '    <span class="cf-header-hint">F · P · H · T · E · ? · Esc</span>',
      ...
      '      <button id="cf-edit-toggle" class="cf-btn" title="Toggle edit mode (E)">✏️ edit text</button>',
      ...
      '    </div>',
      '    <p class="cf-hint">Highlight any text to comment on it. Or click <em>select element</em>, then click any block on the page (image, table, paragraph, section). Shift-click to add more elements. Click <em>edit text</em> to directly fix typos / wording — plain text only, applied instantly. Esc cancels.</p>',
      '  </div>',
  ...
  function onSelectionChange() {
    if (elementMode || editMode) { hideTextPopup(); return; }
  ...
  // ---------------- Edit mode (direct text edits) ----------------
  //
  // Goal: every visible piece of text on the page should be editable, without
  // ever making a structural element (a section/wrapper containing other
  // blocks) contenteditable. The rule:
  //
  //   - Tags we NEVER make editable: script, style, head/body wrappers,
  //     interactive widgets, replaced elements, and our own UI.
  //   - "Safe inline" tags are allowed AS CHILDREN of an editable element
  //     (they ride along inside its inner HTML).
  //   - An element is editable iff it has at least one non-whitespace text
  //     node directly inside, AND every descendant is either text or a
  //     safe-inline tag.
  //   - When two nested elements both qualify, we pick the OUTER one — so the
  //     user clicks once and edits the whole phrase, not just the inline span.
  //
  // Result: <h1>The complete <span>blueprint</span></h1> → the <h1> is the
  // editable unit, the <span> is along for the ride. <span class="label">
  // Executive Overview</span> with no editable parent → the <span> itself.

  const EDITABLE_INLINE_TAGS = new Set([
    "EM", "STRONG", "B", "I", "U", "SPAN", "A", "CODE", "MARK", "SMALL",
    "SUB", "SUP", "BR", "ABBR", "CITE", "KBD", "SAMP", "VAR", "TIME", "Q",
    "DFN", "BDI", "BDO", "WBR", "INS", "DEL", "S",
  ]);

  // Tags that are never editable, even if they happen to contain text.
  // Includes form widgets (have their own value model), replaced elements,
  // and metadata/script/style.
  const NON_EDITABLE_TAGS = new Set([
    ...
    "IFRAME", "OBJECT", "EMBED", "CANVAS", "SVG", "MATH",
    "PROGRESS", "METER",
  ]);
  ...
  // All descendants (recursively) are either text or safe-inline tags.
  function descendantsAreSafeInline(el) {
    for (const child of el.children) {
      if (!EDITABLE_INLINE_TAGS.has(child.tagName)) return false;
      if (!descendantsAreSafeInline(child)) return false;
    }
    return true;
  }
  ...
  function isEditableLeaf(el) {
    ...
    // Custom elements (contain hyphen) — be conservative; their internals may
    // be shadow DOM and we don't want to bork them.
    if (el.tagName.includes("-")) return false;
    // Hidden elements aren't visible text — skip.
    if (el.hidden) return false;
    if (!hasDirectText(el)) return false;
    if (!descendantsAreSafeInline(el)) return false;
    return true;
  }

  function toggleEditMode() {
    editMode = !editMode;
    document.body.classList.toggle("cf-edit-mode", editMode);
    const btn = $("cf-edit-toggle");
    btn.classList.toggle("cf-active", editMode);
    btn.textContent = editMode ? "✓ edit mode (on)" : "✏️ edit text";
    if (editMode) {
      // Turn off element/text selection modes to avoid mode collisions.
      if (elementMode) toggleElementMode();
      hideTextPopup();
      enableEditableLeaves();
      showToast("Click any text block and edit. Plain text only. Blur (click away) to save.", 4500);
    } else {
      disableEditableLeaves();
    }
  }

  function enableEditableLeaves() {
    // Walk the tree top-down. For each element, if it's an editable leaf,
    // mark it and DON'T descend (its descendants are inline-safe and ride
    // along inside it). Otherwise, recurse into its children. This guarantees
    // we always pick the OUTERMOST editable element for any given text.
    const skip = new Set(["SCRIPT", "STYLE", "NOSCRIPT", "TEMPLATE", "IFRAME", "SVG", "MATH"]);
    function walk(el) {
      ...
      if (isEditableLeaf(el)) {
        makeEditable(el);
        return; // do not descend — its inline children ride along
      }
  ...
  function disableEditableLeaves() {
    document.querySelectorAll(".cf-editable").forEach((el) => {
      el.removeAttribute("contenteditable");
      el.removeAttribute("spellcheck");
      el.classList.remove("cf-editable");
  ...
  // Strip formatting on paste — keep edits strictly plain text. Without this,
  // pasting from Word/Notion/etc. injects spans/styles, which we'd then have
  // to reject server-side (angle brackets in textContent → 400). This makes
  // the failure mode invisible: paste just works as plain text.
  ...
  function onEditFocus(e) {
    editOriginalText.set(e.currentTarget, e.currentTarget.textContent);
  }

  function onEditKeydown(e) {
    if (e.key === "Escape") {
      // Revert and blur without saving.
      const orig = editOriginalText.get(e.currentTarget);
      if (orig != null) e.currentTarget.textContent = orig;
      e.currentTarget.blur();
    } else if (e.key === "Enter" && !e.shiftKey) {
      // Single-line commit (multi-line edits inside one <p> are rare).
      e.preventDefault();
      e.currentTarget.blur();
    }
  }

  // Reduce a (oldFull, newFull) pair to the minimal changed substring by
  // stripping the longest common prefix and suffix. Without this, the server
  // can fail to locate the text because the surrounding element may contain
  // inline tags (<span>, <em>, <a>) whose markup splits the source text.
  function diffMiddle(oldFull, newFull) {
    let i = 0;
    const min = Math.min(oldFull.length, newFull.length);
    while (i < min && oldFull[i] === newFull[i]) i++;
    let j = 0;
    while (
      j < (oldFull.length - i) &&
      j < (newFull.length - i) &&
      oldFull[oldFull.length - 1 - j] === newFull[newFull.length - 1 - j]
    ) j++;
    let oldDiff = oldFull.slice(i, oldFull.length - j);
    let newDiff = newFull.slice(i, newFull.length - j);
    // Pure insertion (oldDiff empty) → expand anchor by one char into the
    // surrounding context so the server has something to find in the source.
    if (!oldDiff) {
      if (i > 0) {
        i -= 1;
        oldDiff = oldFull.slice(i, oldFull.length - j);
        newDiff = newFull.slice(i, newFull.length - j);
      } else if (j > 0) {
        j -= 1;
        oldDiff = oldFull.slice(i, oldFull.length - j);
        newDiff = newFull.slice(i, newFull.length - j);
    ...
    const prefix = oldFull.slice(0, i);
    const suffix = oldFull.slice(oldFull.length - j);
    return { oldDiff, newDiff, prefix, suffix };
  }

  // Count how many times `needle` appears in the page's visible text BEFORE
  // the edited element's own occurrence (which sits inside `localPrefix`).
  // Walks the DOM in document order, accumulating textContent from skippable
  // siblings/ancestors, then counts occurrences inside `localPrefix` for the
  // portion within the edited element. Server uses this index to pick the
  // right occurrence in its source file. Skips our own UI and script/style.
  function countOccurrencesBefore(el, localPrefix, needle) {
    if (!needle) return 0;
    let count = 0;
    let stop = false;
    function walk(node) {
      if (stop) return;
      if (node === el) {
        // Reached the edited element — count occurrences in the part of its
        // own text that lies before the edit, then stop.
        count += countAll(localPrefix, needle);
        stop = true;
        return;
      }
      if (node.nodeType === 3) {
        count += countAll(node.nodeValue || "", needle);
        return;
      }
      if (node.nodeType !== 1) return;
      const tag = node.tagName;
      if (tag === "SCRIPT" || tag === "STYLE" || tag === "NOSCRIPT" || tag === "TEMPLATE") return;
      if (insideOurUI(node)) return;
  ...
  async function onEditBlur(e) {
    ...
    const oldText = editOriginalText.get(el);
    editOriginalText.delete(el);
    if (oldText == null) return;
    const newText = el.textContent;
    if (newText === oldText) return;
    // Send only the minimal changed substring. If we sent the entire element
    // text, the server wouldn't find it in the source when the element
    // contains inline children (e.g. <span>, <em>).
    const { oldDiff, newDiff, prefix } = diffMiddle(oldText, newText);
    if (!oldDiff && !newDiff) return;
    // Disambiguate by occurrence index: count how many times `oldDiff` has
    // already appeared in the page's visible text BEFORE the spot we just
    // edited. The server walks its source file in the same document order,
    // skips that many occurrences, and edits the next one. This is exact —
    // no whitespace math, no surrounding context required.
    const occurrence_index = countOccurrencesBefore(el, prefix, oldDiff);
    // If the user accidentally created HTML (e.g. pasted formatted text),
    // bail out: server rejects anything with angle brackets anyway, but we'd
    // rather warn here than make a confusing round-trip.
    if (el.innerHTML !== escapeHtml(newText) && /[<>]/.test(newText)) {
      showToast("Edit rejected: plain text only (no HTML).", 3500);
      el.textContent = oldText;
      return;
    }
    // Anchors are optional: the server's text-anchored fallback finds the
    // minimal diff inside <body> directly. We still send id/cf_id when
    // available so the server can scope its search and disambiguate when
    // the same text appears multiple times.
    const cf_id = el.dataset.cfId || null;
    ...
    try {
      ...
      if (!resp.ok || !data.ok) {
        ...
        showToast("Edit failed: " + err, 4500);
        el.textContent = oldText;
        return;
      }
      showToast("Saved.", 1800);
      // history.json will be refreshed by the regular poll and the
      // walkthrough will flag the change.
    } catch (err) {
      console.error(err);
      showToast("Edit failed: " + err.message, 4500);
      el.textContent = oldText;
    }
  }
  ...
  function bindEvents() {
    ...
    $("cf-edit-toggle").addEventListener("click", toggleEditMode);
    ...
    // While in edit mode, swallow clicks on links / buttons inside editable
    // regions so the user can place their caret without navigating away.
    document.addEventListener("click", (e) => {
      if (!editMode) return;
      const t = e.target;
      if (!t || !t.closest) return;
      if (insideOurUI(t)) return;
      if (t.closest(".cf-editable")) {
    ...
    document.addEventListener("keydown", (e) => {
      ...
      if (e.key === "Escape") {
        ...
        else if (editMode && !isTypingTarget(e.target)) toggleEditMode();
      ...
      switch (e.key) {
        ...
        case "?":
          e.preventDefault();
          showToast("F: feedback · P: pending · H: history · T: tour · E: edit text · R: reload when changes ready · ←/→: tour nav · Esc: close", 6500);
          break;
      }
    });
  }
