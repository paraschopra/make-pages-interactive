...
(function () {
  ...
  async function fetchStatus() {
    ...
    try {
      ...
      const text = await resp.text();
      if (text === lastStatusString) return;
      ...
      // A status entry for one of our in-flight comments is proof an agent is
      // alive and working — mirror the stale-timer push-back fetchHistory does
      // when history.json changes. Also exit stale state if we'd already
      // flipped (e.g., agent posted status late). Without this, any task
      // longer than STALE_AFTER_MS would hit the "no agent picked this up"
      // banner mid-flight — the exact failure mode this feature was meant to
      // prevent.
      const liveStatus = lastSubmittedBatch.comment_ids.some(
        cid => currentStatus[cid] && currentStatus[cid].message
      );
      if (liveStatus) {
        if (staleTimer) clearTimeout(staleTimer);
        staleTimer = setTimeout(() => {
        ...
        if (isBatchStale) isBatchStale = false;
      }
      renderPending();
    } catch (e) { /* network glitch */ }
  }
